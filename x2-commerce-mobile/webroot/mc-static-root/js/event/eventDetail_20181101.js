...
monthEvent.detail = {
    ...
    init : function(){
        ...
        if(common.isLogin()){
            ...
            monthEvent.detail.checkAttendCheckDayFirst("31");
    ...
    addMyStmp : function(){
        if (common.app.appInfo == undefined || !common.app.appInfo.isapp) {
            ...
            if(!mevent.detail.checkLogin()){
                ...
                common.Ajax.sendJSONRequest(
                        "GET"
                      , _baseUrl + "event/20181101/addMyStmpJson.do"
                      , param
    ...
    getApplyEvent : function() { 
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20181101/getApplyEventJson.do"
              , param
...
//            alert("//==>"+json.applyOne+"//==>"+json.applyTwo+"//==>"+json.applyThree);
    ...
    getMyStmpEvent : function(){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20181101/getMyStmpEventJson.do"
              , param
                           ...
                           for(var i=0; i<newCheckList.length; i++){
                               if(index == i){
                                      if(newCheckList[i].strtDtime == 1 ){
                                          var htmlStr = "";
                                          htmlStr =  "<img src='"+_cdnImgUrl + "contents/201811/01attend/attend_stamp.png' alt='출석'>";
...
//                                          monthEvent.detail.newCheckListChk = i;
    ...
    checkAttendCheckDayFirst:function(fvrSeq){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20181101/checkAttendCheckDayJson.do" // ID 당 1개의 응모번호만 응모가능 (1. 난수번호체크 , 2.ID당1번만응모했는지체크)
              , {
                      evtNo : $("input[id='evtNo']:hidden").val()
                    , fvrSeq : fvrSeq
                    , noteContFlag : "Y"
                        ...
                        if(json.ret == "0"){
                            if( json.myTotalCnt  == "0" ) {  // 10일연속, 20일연속, 30일연속  세개중 한번도 신청하지 않은경우 위수탁 받기 
                              //  $("div#Confirmlayer1").attr("onClick", "monthEvent.detail.applyAttendCheckDayInfo(' " + fvrSeq + "');");
                                ...
                                monthEvent.detail.eventShowLayer2('eventLayerPolice');
                            }else {
    ...
    applyAttendCheckDayInfo : function(fvrSeq,myTotalCnt){
        if(common.app.appInfo == undefined || !common.app.appInfo.isapp){
            ...
            common.Ajax.sendJSONRequest(
                    "GET"
                  , _baseUrl + "event/20181101/applyAttendCheckDayInfo.do"  
                  , param
