...
monthEvent.detail = {
    ...
    addMyStmp : function(){
        if (common.app.appInfo == undefined || !common.app.appInfo.isapp) {
            ...
            if(!mevent.detail.checkLogin()){
                ...
                common.Ajax.sendJSONRequest(
                        "GET"
                      , _baseUrl + "event/20190101/addMyStmpJson.do"
                      , param
    ...
    getApplyEvent : function() {
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20190101/getApplyEventJson.do"
              , param
...
//            alert("//==>"+json.applyOne+"//==>"+json.applyTwo+"//==>"+json.applyThree);
    ...
    getMyStmpEvent : function(){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20190101/getMyStmpEventJson.do"
              , param
                           ...
                           for(var i=0; i<newCheckList.length; i++){
                               if(index == i){
                                      if(newCheckList[i].strtDtime == 1 ){
                                          var htmlStr = "";
                                          htmlStr =  "<img src='"+_cdnImgUrl + "contents/201901/01attend/attend_stamp.png' alt='출석'>";
...
//                                          monthEvent.detail.newCheckListChk = i;
    ...
    checkAttendCheckDayFirst:function(fvrSeq){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20190101/checkAttendCheckDayJson.do" // ID 당 1개의 응모번호만 응모가능 (1.ID당1번만응모했는지체크)
              , {
                      evtNo : $("input[id='evtNo']:hidden").val()
                    , fvrSeq : fvrSeq
                }
    ...
    checkAttendCheckDay:function(fvrSeq){
        if(common.app.appInfo == undefined || !common.app.appInfo.isapp){
            ...
            common.Ajax.sendJSONRequest(
                    "GET"
                  , _baseUrl + "event/20190101/checkAttendCheckDayJson.do" // ID 당 1개의 응모번호만 응모가능 (1. 난수번호체크 , 2.ID당1번만응모했는지체크)
                  , {
                          evtNo : $("input[id='evtNo']:hidden").val()
                        , fvrSeq : fvrSeq
                        , chkPushYn : 'Y'
    ...
    applyAttendCheckDayInfo : function(fvrSeq,myTotalCnt){
        if(common.app.appInfo == undefined || !common.app.appInfo.isapp){
            ...
            common.Ajax.sendJSONRequest(
                    "GET"
                  , _baseUrl + "event/20190101/applyAttendCheckDayInfo.do"
                  , param
