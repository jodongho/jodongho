...
mevent.detail = {
	...
	initSns : function(){
//        mevent.detail.url = window.location.protocol + "//" + window.location.host + window.location.pathname + "?evtNo="+$("#evtNo").val();
        mevent.detail.url = _baseUrl + 'E.do?evtNo=' + $("#evtNo").val();
        if($("#evtNo").val() == "00000000005220" || $("#evtNo").val() == "00000000005397" || $("#evtNo").val() == "00000000005788"){
            common.sns.init( $("#bnrImgUrlAddr").val(), "올리브영 온라인몰 생일 초대장 도착! 응모권 받고, 100% 혜택 기대해♥", mevent.detail.url);
        }else{
            common.sns.init( $("#bnrImgUrlAddr").val(),$("#evtNm").val(), mevent.detail.url);
        }
    },
    initEvNo : function(){
        mevent.detail.evtNo = $("#evtNo").val();
    },
    init : function(){
        ...
        $("#commmentHeader > .more").click(function(){
            ...
            mevent.detail.getEventMyCommentListAjax();

        });

        $("#commmentHeader > .tit").click(function(){
            $('#commentList').empty();
            mevent.detail.getEventCommentListAjax();
        });
        ...
        $('.btnGreen').click(function(){
            ...
            if(regType == 'regWrite'){
                if(!mevent.detail.checkRepl()){
                    return;
                }

                mevent.detail.regEventCommentAjax();
            }else{

                if(!mevent.detail.checkRegAvailable()){
                    return;
                }
                if(!mevent.detail.checkAgrmInfo()){
                    return;
                }

                mevent.detail.regEventAjax();
            }
		...
		$('.btnGreen2').click(function(){
			...
			if(regType == 'regWrite'){
				...
				mevent.detail.regReceiptEventAjax();
			}
		});
		...
		mevent.detail.initEvNo();
		mevent.detail.initCommentList();

		if((location.href).indexOf("event/getEventDetail.do") > 0){
			/* 이벤트 기본 디테일에서만 활성화 */
			mevent.detail.initSns();
    ...
    initCommentList : function(){
        ...
        if(evtType == mevent.detail.commenttype || rplYn ==  "Y"){
            mevent.detail.getEventCommentListAjax();
        }
    ...
    commentModifyMain : function(appenArea,bbcSeq,commentInfo){
        $("#regComment").hide();
        mevent.detail.makeCommentModify(commentInfo,appenArea,bbcSeq)
    },

    cancelModifyMain : function(showArea){
        showArea.show();
        mevent.detail.removeCommentModify(showArea);
        $("#regComment").show();
    },
    ...
    makeCommentModify : function(commentInfo,showArea, bbcSeq){
        ...
        $buttonModify.click(function(){
            ...
            mevent.detail.uptEventCommentAjax(bbcSeq,cont);
        });
        ...
        $buttonCancel.click(function(){
            ...
            mevent.detail.cancelModifyMain(showArea);
        });
          ...
          PagingCaller.init({
              callback : function(){
                  ...
                  common.Ajax.sendRequest(
                          ...
                          , param
                          , mevent.detail._callback_getEventCommentListAjax);
              }
          ,startPageIdx : 0
          ,subBottomScroll : 700
          ,initCall : true
    ...
    _callback_getEventCommentListAjax : function(strData) {
        ...
        if(listCount == 0){
            ...
            $.each(list, function(index, element){
                ...
                if(rplYn == 'Y' && type != mevent.detail.commenttype && $.trim(element.myCommentFlag) == 'Y'){
                    var $divBox = $("<div>").addClass("btn_box");
                    $li.append($divBox);

                    var $buttonModify = $("<button>").addClass("btnGrayH28").text("수정");
                    $buttonModify.click(function(){
                        mevent.detail.commentModifyMain($(this).parent().parent(),element.bbcSeq,fcont);
                    });
                    $divBox.append($buttonModify);

                    var $buttonCancel = $("<button>").addClass("btnGrayH28").text("삭제");
                    $buttonCancel.click(function(){
                        mevent.detail.delEventCommentAjax(element.bbcSeq);
                    });
    ...
    _callback_getEventMyCommentListAjax : function(strData) {
        ...
        if(listCount == 0){
            ...
            $.each(list, function(index, element){
                ...
                if(type != mevent.detail.commenttype){
                    var $divBox = $("<div>").addClass("btn_box");
                    $li.append($divBox);
    ...
    regEventAjax : function(){
        ...
        var param = {};
        if(type == mevent.detail.billtype){

            var billNum = $("#buyNumber").val();

            if(!mevent.detail.checkBillNumber(billNum)){
                return;
            }
            ...
            param = {
                ...
                evtNo : evtNo,
                evtClssCd : typeCd,
                mbrInfoUseAgrYn : mevent.detail.mbrInfoUseAgrYn,
                mbrInfoThprSupAgrYn : mevent.detail.mbrInfoThprSupAgrYn,
                chkPushYn : $("#mbrAppPushAplyYn").val()    //APP PUSH 동의(온라인 이벤트 유형 추가)
            };

        }else if(type == mevent.detail.commenttype){

            var bbcFcont = $("#inpTxBox").val();

            if(!mevent.detail.checkCommentInfo(bbcFcont)){
                return;
            }
    ...
    checkAgrmInfo : function(){
        ...
        if(regYn == 'Y' && mbrInfoUseAgrYn == 'Y'){
            ...
            if(!$("#chk01").is(":checked")){
                ...
                mevent.detail.mbrInfoUseAgrYn = 'Y';
            }
        }else{
            mevent.detail.mbrInfoUseAgrYn = 'N';
        }
        ...
        if(regYn == 'Y' && mbrInfoThprSupAgrYn == 'Y'){
            if(!$("#chk02").is(":checked")){
                ...
                mevent.detail.mbrInfoThprSupAgrYn = 'Y';
            }
        }else{
            mevent.detail.mbrInfoThprSupAgrYn = 'N';
        }
...
//
        ...
        $inputUrl.html(mevent.detail.url);
    ...
    downAppCouponEventJson : function(cpnNo){
        if(common.app.appInfo == undefined || !common.app.appInfo.isapp){
            ...
            mevent.detail.downCouponJson(cpnNo);
        }
    },
	...
	updateTermsAgrInfoAjax : function(param){
		common.Ajax.sendJSONRequest(
			  ...
			  , param
			  , mevent.detail._callback_updateTermsAgrInfoAjax
		);
	},
	...
	mktRcvSend : function(smsYn){
    	...
    	var data = {
            ...
            agr40 : 'Y',
    		pushMsgRcvYn : 'Y',
    		smsRcvAgrYn : (smsYn == 'Y') ? 'Y' : 'N'
    	};
    	
        common.Ajax.sendRequest("POST", url, data, mevent.detail._callBack_mktRcvSend);
    },
    
    _callBack_mktRcvSend : function (data){
        if (data.result) {
        	mevent.detail.sendTmsPush(data.pushYn); // TMS 푸시동의정보 송신
    ...
    sendTmsPush : function(push){
		...
		common.Ajax.sendRequest("POST",url,data, mevent.detail._callbackSendTmsPush);
    },
