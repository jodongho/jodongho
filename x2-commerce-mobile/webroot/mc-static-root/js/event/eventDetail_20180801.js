...
monthEvent.detail = {
        ...
        if (common.app.appInfo == undefined || !common.app.appInfo.isapp) {
            ...
            if(!mevent.detail.checkLogin()){
                ...
                common.Ajax.sendJSONRequest(
                        "GET"
                      , _baseUrl + "event/20180801/addMyStmpJson.do"
                      , param
    ...
    _callback_addMyStmpJson : function(json){
        if(json.ret == "0"){
            if(json.winYn=="Y"){
                mevent.detail.eventShowLayer('evtPoint');
            }else{
    ...
    getApplyEvent : function() { 
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20180801/getApplyEventJson.do"
              , param
...
//            alert("//==>"+json.applyOne+"//==>"+json.applyTwo+"//==>"+json.applyThree);
            /* 신청되었으면 비활성화 */
            if(json.applyOne == 1){
                $("div#chul3_1").addClass("none");
            }
            if(json.applyTwo == 1){
                $("div#chul3_2").addClass("none");
            }
            if(json.applyThree == 1){
                $("div#chul3_3").addClass("none");
            }
        } else {
            alert(json.message);
        }
    ...
    getMyStmpEvent : function(){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20180801/getMyStmpEventJson.do"
              , param
                               ...
                               for(var i=0; i<newCheckList.length; i++){
                                   if(index ==   i){
                                          if(newCheckList[i].strtDtime == 1 ){
                                              var htmlStr = "";
                                              htmlStr =  "<img src='"+_cdnImgUrl + "contents/201808/01attend/chulcheck_stamp.png' alt='출석'>";                                              
    ...
    checkAttendCheckDayFirst:function(fvrSeq){
        ...
        common.Ajax.sendJSONRequest(
                "GET"
              , _baseUrl + "event/20180801/checkAttendCheckDayJson.do" // ID 당 1개의 응모번호만 응모가능 (1. 난수번호체크 , 2.ID당1번만응모했는지체크)
              , {
                      evtNo : $("input[id='evtNo']:hidden").val()
                    , fvrSeq : fvrSeq
                }
              , function(json){ 
                  ...
                  $(':radio[name="argee2"]:checked').attr("checked", false);
                    ...
                    if(json.ret == "0"){
                        if(fvrSeq == "32"){
                            $("div#chul3_1").removeClass("none");
                            $("div#chul3_1").removeClass("miss");
                        }
                        if(fvrSeq == "33"){
                            $("div#chul3_2").removeClass("none");
                            $("div#chul3_2").removeClass("miss");
                        }
                        if(fvrSeq == "34"){
                            $("div#chul3_3").removeClass("none");
                            $("div#chul3_3").removeClass("miss");
                        }
                    }else{
                        if(fvrSeq == "32"){
                            $("div#chul3_1").removeClass("none");
    ...
    applyAttendCheckDayInfo : function(fvrSeq,myTotalCnt){
        if(common.app.appInfo == undefined || !common.app.appInfo.isapp){
            ...
            common.Ajax.sendJSONRequest(
                    "GET"
                  , _baseUrl + "event/20180801/applyAttendCheckDayInfo.do"  
                  , param
