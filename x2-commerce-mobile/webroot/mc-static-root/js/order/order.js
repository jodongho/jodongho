...
morder.orderForm.dlvpInfo = {
    ...
    init : function() {
        ...
        $('#dlvpSelect').change(function() {
             ...
             * 24H 배송인 경우, 배송지 변경 
             */            
            if ($("#is24HCheckable").val() == "Y" && morder.orderForm.o2o24h.initCompleted) {            	
        		var addr = eval('deliveryList.delivery_'+$("#dlvpSelect").val());	
        		addr.postNo = morder.orderForm.Base64.decode(addr.rmitPostNo);		// 우편번호
        		
		        // MO 증복실행 방지 
        		if(addr.postNo != morder.orderForm.o2o24h.postNo ||
        				addr.emdNm != morder.orderForm.o2o24h.emdNm ||
        				addr.admrNm != morder.orderForm.o2o24h.admrNm ) {
        			
        			morder.orderForm.o2o24h.postNo = addr.postNo;		// 우편번호
        			morder.orderForm.o2o24h.emdNm = addr.emdNm;			// 법정동
        			morder.orderForm.o2o24h.admrNm = addr.admrNm;		// 행정동
        			
        			morder.orderForm.o2o24h.changeDelivery24h(true, addr);
        		}
            }
        });
        ...
        */
        $("#mbrMemoCont_exist, #mbrMemoCont_new").find("li a[cd]").click(function(){
            ...
            if(cd == "99"){
            	...
            	//오늘드림 주문 최근 배송메시지 선택시 input박스에 해당내용을 표시
            	$("[name='mbrMemoCont']").val("");
                $("[name='mbrMemoCont']").val($(this).text().replace("(최근) ", ""));
                $("#msgCnt_exist").text($("[name='mbrMemoCont']").val().length);
                $("[name='mbrMemoContDiv']").show();
            }else {
                $("[name='mbrMemoContDiv']").hide();
        ...
        $("input[name='o2oDlvSp']").change(function(){
            ...
            if($(this).val() == "1"){
                ...
                $("#mbrMemoCont_exist").find("li a[cd=O2O]").show();
	                ...
	                $("#mbrMemoCont_exist").find("li a[cd]").hide();
	                $("#mbrMemoCont_exist").find("li a[cd]").eq(0).show();
	                $("#mbrMemoCont_exist").find("li a[cd=MH]").show();
                }
            ...
            /**
             * 2020-08-10 o2oJJ
             * hide 처리되어있는 24H 를 클릭처리하여 기존 오늘드림의 프로세스를 그대로 실행한다.
             */
            else if ($(this).val() == "6") {
            	if(to_hours <= 16){
                    $("#o2oDeliveryScheduleDt").val(sysDate.format("yyyyMMdd")+ "" + "173000");
                    $("#dlvpDueDate").text(sysDate.format("yyyy년 MM월 dd일 PM5"));
                }else{
                    sysDate.setDate(sysDate.getDate()+1);
                    $("#o2oDeliveryScheduleDt").val(sysDate.format("yyyyMMdd")+ "" + "173000");
                    $("#dlvpDueDate").text(sysDate.format("yyyy년 MM월 dd일 PM5"));
    ...
    getCallback : function(target) {
        ...
        var callback_zipcode = function(deliveryInfo) {
          ...
          //주소정보 세팅
        	...
        	// 2020-08-18 o2oJJ 신규배송지 우편번호 찾기일 경우 제외처리함.            
            if ($("#is24HCheckable").val() == "Y" && morder.orderForm.o2o24h.initCompleted) {
            	var delivery = { postNo: deliveryInfo.postNo,
            					 emdNm: deliveryInfo.emdNm,            				     
            				     admrNm: deliveryInfo.admrNm };
            	var isExistTab = (target == "exist");
            	morder.orderForm.o2o24h.changeDelivery24h(isExistTab, delivery);
            }
        }
...
morder.orderForm.coupon = {
        ...
        $("input[name='Discount_Benefits']").change(function(){
            if($(this).val() == "auto") {
             ...
             // 혜택 직접 선택의 경우 오늘드림 배송 쿠폰 셋팅
                if($("#orgQuickYn").val() == "Y") { //오늘드림 배송일 경우
    ...
    changeCoupon : function(index, _selectedOption) {
        ...
        //포인트 사용 금액 초기화 bmiy20
        if(Number($("input[name='cjonePntNonPosCnt']").val()) > 0 && Number($("input[name='cjonePntAplyAmt']").val()) > 0) {
        	morder.orderForm.point.initPoint();
	        alert("쿠폰이 변경되어 적용된 포인트를 초기화 합니다.");
        }
...
//                return Number(b.rtAmtVal) - Number(a.rtAmtVal);
            ...
            // 각 상품별 최대할인 쿠폰 세팅
            // 2020-08-14 o2oJJ 혜택 직접 선택하기시 세팅 안함.
            if ($("input[name=Discount_Benefits]:checked").val() == "auto") {
        ...
        $("#paymentCouponList select[index]").each(function(){
            ...
            // 상품쿠폰 적용 후 주문쿠폰이 상품총합이 넘지 않을 경우에만 주문쿠폰 적용 처리 bmiy20
            if(maxRtAmtVal > 0 && _payAmt.totGoodsAmt >= maxRtAmtVal) {
                if($("#orgQuickYn").val() == "Y") {
    ...
    setDwnldCoupon : function(index, aplyYn) {
        ...
        //상품 할인 금액 + 주문별 할인 합쳐서 상품주문금액보다 크지 않을 경우에만 할인 쿠폰 적용 처리 하도록 수정을 하고 싶구나 2020.07.23 bmiy20  
        var totDscntAmt = 0;        
        $("input[name='cpnDscntAmt']").each(function(i){
        	totDscntAmt += Number($(this).val());
        });
        
        if (morder.orderForm.payAmt.totGoodsAmt < totDscntAmt && Number(cpnDscntAmt) > 0) {
            ...
            $("input[name='couponList["+index+"].dwnldGoods1DcAmt']").val("");
            
            $("#cpnDscntAmt_"+index).val(0);
            $("#cpnDscntAmt_"+index+"_span").text(0);
            
   			$("select[name='couponList["+index+"].promChk']").val("N").prop("selected",true);    		
    	}
    },
    ...
    setPaymentCoupon : function(index, aplyYn) {
        ...
        if(aplyYn == 'Y') {
            ...
            if(couponInfo != null) {
            	...
            	if (morder.orderForm.payAmt.totGoodsAmt < totDscntAmt && Number(couponInfo.rtAmtVal) > 0) {            		
            		alert("할인금액은 상품가격을 초과할 수 없습니다.\n쿠폰할인정보를 확인해주세요.");
...
//                        $("#giftNoti").hide();
    ...
    resetCoupon : function(){
        ...
        morder.orderForm.payAmt.setCpnDscntAmt();  
        var totCpnAplyAmt = morder.orderForm.payAmt.cpnDscntAmt;
    ...
    valid : function() {
    	var validObj = new morder.orderForm.validObj();
        validObj.element= $("#couponList[0].promChk");
    	validObj.isValid = true;  
        validObj.validMsg = "";
             
        var _payAmt = morder.orderForm.payAmt;
        // 총 할인금액 (즉시할인 + 쿠폰할인)
        var totDscntAmt = Number(_payAmt.imdtDscntAmt) + Number(_payAmt.cpnDscntAmt);
        
        //상품금액보다 쿠폰 사용액이 큰 경우 2020.07.23 
        if ((Number(_payAmt.totGoodsAmt) + Number(_payAmt.giftBoxAmt)) < Number(totDscntAmt))  {      	
        	validObj.validMsg = "할인금액은 상품가격을 초과할 수 없습니다.\n쿠폰할인정보를 확인해주세요.";   
        	morder.orderForm.coupon.resetCoupon();
        	validObj.isValid = false;
        }
        ...
        if(_paymentCouponList.size() > 0
            ...
            var callback_getCouponListAjax = function(data){
                if(data.result == "S") {
                    ...
                    if($("#orgQuickYn").val() == "Y" && !$("input[name='o2oDlvSp']").is(":checked")) {
                    ...
                    } else if($("#orgQuickYn").val() == "Y" && $("input[name='o2oDlvSp']").is(":checked")) {
                       var o2oDlvSpVal = $("input[name='o2oDlvSp']:checked").val();
                       $("#paymentCouponList select:has(option[value='Y'])").find("option[value='Y']").each(function() {
                            var o2oCpnSp = $(this).attr("o2ocpnsp");
...
//                            _select.prop("disabled", true);
                            _select.on("click", function(e){
                                alert("결제금액이 변경되어 적용할 수 있는 쿠폰이 없습니다.");
                            });
                        }
                    };
...
//                $("#cafeteriaPnt_span").html("사용불가");
    ...
    activateFormObj : function(_input, _btn, ownPnt, aplyAmt) {
        ...
        var fnCjonePosAmt = function(amt){
        	var cjAmt = amt;
            var cjonePntPosAmt = Number($("input[name='dlexPayAmt']").val()); //배송비 더해준 금액(배송비는 포인트 사용 가능)
            var totGoodsSaleAmt = 0;
        	var payCpnAmtSum = 0;
        	var payCpnAmtM = 0;

        	var goodsList = $.map($('.divClass , #goodsInfo'),function( a, b){
           		return {
           			goodsno: $(a).attr('goodsno'),
           		    pntrsrvyn: $(a).attr('pntrsrvyn'),
           		    realsaleprc: $(a).attr('saleprc') * $(a).attr('ordqty'),
           		    totsaleprc : $(a).attr('saleprc') * $(a).attr('ordqty')
           		}
        	});
        	...
        	var prdCpnlist = $.map($("#dwnldCouponList select:not([disabled])"),function( a, b){
              	 return {
              		 goodsno: $(":selected", a).attr('goodsno'),
        	...
        	//주문별 쿠폰
        	var payCpnlist = $.map($("#paymentCouponList select:not([disabled])"),function( a, b){
             	 return {
             	     rtamtval: parseInt($(":selected", a).attr('rtamtval')) ,
             	     value: $(":selected", a).attr('value')          	     
             	 }
        	...
        	//장바구니 쿠폰 금액 sum
        	for(var idx=0; idx < payCpnlist.length; idx++) {
        		if (payCpnlist[idx].value == "Y") {
        			payCpnAmtSum += payCpnlist[idx].rtamtval; 
        		}
        	}  
        	
        	//상품별 쿠폰 사용 후 포인트 사용 가능 금액 구하기        	
        	for(var i=0; i < goodsList.length; i++) {
        	    for(var j=0; j < prdCpnlist.length; j++ ) { //상품쿠폰이 cjone 포인트 적용 불가 상품의 쿠폰인지 확인
        			if (goodsList[i].goodsno == prdCpnlist[j].goodsno && prdCpnlist[j].value == "Y"){ //사용하는 쿠폰
    	        	   goodsList[i].realsaleprc = Number(goodsList[i].totsaleprc) - Number(prdCpnlist[j].rtamtval);
            	   }
    		    }
                if (goodsList[i].pntrsrvyn == "Y"){ 
            	   cjonePntPosAmt += Number(goodsList[i].realsaleprc);
                }
                //쿠폰 금액을 제외한 상품 총 금액
                totGoodsSaleAmt += Number(goodsList[i].realsaleprc);
        	}
        	
        	//장바구니 쿠폰 금액 중 포인트 적립 불가 상품에 대한 할인액 구하기
        	if (payCpnAmtSum > 0) {
	    		for(var g=0; g < goodsList.length; g++) {
	    			if (goodsList[g].pntrsrvyn == "Y") {
	    				payCpnAmtM += payCpnAmtSum * (Number(goodsList[g].realsaleprc) /totGoodsSaleAmt);
	    			}        			
	    	 	}   	
        	}        	
        	
        	//포인트 사용 금액 = (상품별 쿠폰 사용 후 적립 가능액 - 적립가능 상품 장바구니 쿠폰 할인 안분액) + 포인트사용불가 상품의 장바구니 쿠폰 할인 안분액
        	cjonePntPosAmt = cjonePntPosAmt-payCpnAmtM;
        	
        	//포인트 사용하려 입력한 금액이 0보다 크고 포인트 사용 가능금액 금액보다 클때 
        	if (cjAmt > 0 && cjAmt > cjonePntPosAmt) {
        		cjAmt = cjonePntPosAmt;
        	}
...
morder.orderForm.payAmt = {
    ...
    totPayAmt               : 0,
    ordPayAmt				: 0, // 2020-08-10 o2oJJ 포인트를 제외한 결재 금액
    ...
    initPayAmt : function(){
        ...
        // 24H 배송 결제금액 체크
        if ($("#is24HCheckable").val() == "Y" && morder.orderForm.o2o24h.initCompleted) {
	        var ordPayAmt24h = Number(_this.totGoodsAmt) - Number(totDscntAmt);
	        var gitfCnt24h = Number($("#giftCount").val());
	        if(isNaN(gitfCnt24h)) gitfCnt24h = 0;        
	        morder.orderForm.o2o24h.applyPayAmt24h(ordPayAmt24h, gitfCnt24h);
        }
...
morder.orderForm.payMethod = {
    ...
    init : function() {
        ...
        this.savePayMeanCdLog = $("#savePayMethodYn").attr("paymeancd");
...
//            showCartDscnt($(this).val());
    ...
    valid : function() {
        ...
        if(totPayAmt != 0){
            ...
            if(!payMeanCd){
                ...
                // 무통장입금
                	...
                	if($("input[name='o2oDlvSp']:checked").val() == "6") {
                		validObj.isValid = false;
                		validObj.validMsg = "오늘드림 배송은 무통장 입금 이용이 불가합니다.";
                		validObj.element = $("#payMethod");
                		validMsgArray.push(validObj);
                	}
        ...
        //2020.07.10 ds 시나리오 추가 bmiy20
        common.wlog("order_payMethod_"+payWlogId);   
        n_click_logging( _baseUrl + "?clickarea=order_payMethod_"+payWlogId);
...
// 2020-08-10 o2oJJ
// 24H 기능 추가
$.namespace("morder.orderForm.o2o24h")
morder.orderForm.o2o24h = {
	...
	emdNm  : '', 
	admrNm : '',	
	firstDeliveryChecked: false,
	init: function() {
		
		//console.log("--> o2o24h called init.");
		
		var _this = morder.orderForm.o2o24h;
		
		_this.ordPayAmt = morder.orderForm.payAmt.totPayAmt;
		//_this.giftCount = Number($("#giftCount").val());
		//if(isNaN(_this.giftCount)) _this.giftCount = 0;		
		
		_this.eventBind();
		
		if ($("#orgQuickYn").val() == "Y") {
			// 오늘드림  
			_this.orderDomAccess('3');
			
		} else {
			
			if(Object.keys(deliveryList).length == 0) {
				$("#is24HCheckable").val("N");
			}
			
			/*
			if ($("#is24HCheckable").val() == "Y" && _this.ordPayAmt >= 30000 && _this.giftCount <= 0) {
				
				// 24H주문 
				_this.orderDomAccess('2');
				
				// default 선택된 배송지의 우편번호, 법정동, 행정동을 조회하여 매장 코드 및 재고 체크를 진행한다.		        
				var ssInfo = {result:""};
		        var delivery = eval('deliveryList.delivery_'+$("#dlvpSelect").val());
		        
		        try {
			        _this.postNo = morder.orderForm.Base64.decode(delivery.rmitPostNo);		// 우편번호
			        _this.emdNm = delivery.emdNm;											// 법정동
			        _this.admrNm = delivery.admrNm;											// 행정동
			        
			        ssInfo = _this.findItemStoreStock(_this.postNo, _this.emdNm, _this.admrNm);
		        } catch(error) {
		        	//console.log(error);
		        	_this.orderDomAccess('1');
		        	$("#is24HCheckable").val("N");
		        	return;
		        }
		        
				// * null: 매장 없음, * Y: 권역 매장의 재고 있음, * N: 권역 매장의 재고 없음
				if (ssInfo != 'undefined' && ssInfo != '' && ssInfo.result == "Y") {
					// 24H가능
					$(".o2o_24h_chk_area").show();
					$("#strNo24H").val(ssInfo.strNo);
					
            		if ($("#o2o24HChk").prop("checked") == false) {
            			$("#o2o24HChk").prop("checked", true);
            		}					        
            		
					$("#quickYn").val("Y");
					$("input[name=o2oDlvSp][value='6']").click();
        ...
        console.log("24h_initialized.");
        
	},
	applyPayAmt24h: function (ordPayAmt, giftCount) {
		
		var _this = morder.orderForm.o2o24h;
		
		if (_this.initCompleted == false)
			return;
		
		if (_this.ordPayAmt <= ordPayAmt && _this.giftCount == giftCount) {
			//console.log("skip applyPayAmt24h");
			return;
		}
		
		_this.ordPayAmt = ordPayAmt;
		_this.giftCount = giftCount;
		
		// 오늘드림 본품, (온)증정품없음, 3만원이상  
    	if ($("#is24HCheckable").val() == "Y" ) {

			if(_this.ordPayAmt >= 30000 && _this.giftCount <= 0) {        	        			

    			_this.orderDomAccess('2');	        		
        		if ($("#o2o24HChk").prop("checked")) {
        			$("#o2o24HChk").prop("checked", true);
        		}					        
				...
				// 1:일반주문 
				_this.orderDomAccess('1');
				$(".o2o_24h_chk_area").hide();
				$("#is24HCheckable").val("N");        			
				
        		if ($("#o2o24HChk").prop("checked")) {
        			$("#o2o24HChk").prop("checked", false);
        		}					        
        		$("input[name=o2oDlvSp][value='6']").prop("checked", false);
        		
        		$("#quickYn").val("N"); 
    		}
        }
    	
    	console.log("is24HCheckable : " + $("#is24HCheckable").val());
	},			
	
	eventBind: function() {
		$("#o2o24HChk").on("click", function(e) {
			//console.log(">>>> eventBind click()");
			if (this.checked) {
            	var delivery = { 
            			postNo: morder.orderForm.o2o24h.postNo,
            			emdNm: morder.orderForm.o2o24h.emdNm,
            			admrNm: morder.orderForm.o2o24h.admrNm };
            	morder.orderForm.o2o24h.changeDelivery24h(true, delivery);
			} else {
				morder.orderForm.o2o24h.orderDomAccess("1");
				$("#strNo24H").val("");
				$("input[name=o2oDlvSp][value='6']").prop("checked", false);
			}
		});
		...
		// 우편번호 찾기 제어
		$("#search-zipcode-pop_exist_r").on("click", function(e) {
			// 24H일 경우 우편번호 찾기 클릭시 confirm 창 안내처리
			if ($("#o2o24HChk").prop("checked")) {
				if (confirm("배송지를 변경하시면 오늘드림 이용이 해제될 수 있습니다.\n변경하시겠습니까?")) {
					$("#search-zipcode-pop_exist").trigger("click");
				}
			} else {
				$("#search-zipcode-pop_exist").trigger("click");
			}
	...
	/**
	 * 현재 선택된 배송지에 해당 하는 매장 조회 및 매장 재고 체크
	 * @param postNo: 우편번호
	 * @param emdNm: 법정동
	 * @param admrNm: 행정동
	 */
	findItemStoreStock: function(postNo, emdNm, admrNm) {
		
		var rtn = {};
		// parameter로 전달하기 위한 상품정보 가공
		var paramItems = "";
		var items = morder.orderForm.payAmt.goodsInfoList;
		for (key in items) {
			paramItems += items[key].goodsNo+"||"+items[key].itemNo+"||"+items[key].qty+",";
		}
		
		common.Ajax.sendRequest(
			"POST",
			 _baseUrl + "order/find24StoreInfo.do",
			{
				zipcode:  postNo,
				admrNm: admrNm,
				emdNm: emdNm,
	 ...
	 * 배송지목록 변경시 매장/재고 조회  
	 */
	changeDelivery24h: function (isExistTab, delivery) {
		...
		// 오늘드림 본품, (온)증정품없음, 3만원이상  
    	if ($("#is24HCheckable").val() == "Y" && isExistTab && _this.ordPayAmt >= 30000 && _this.giftCount <= 0) {
	        ...
	        try {
	        	if(delivery == '') {
	        		addr = eval('deliveryList.delivery_'+$("#dlvpSelect").val());	
			        _this.postNo = morder.orderForm.Base64.decode(addr.rmitPostNo);		// 우편번호
			        _this.emdNm = addr.emdNm;			// 법정동
			        _this.admrNm = addr.admrNm;			// 행정동
			        
	        	} else {
	        		_this.postNo = delivery.postNo;
	        		_this.emdNm = delivery.emdNm;
	        		_this.admrNm = delivery.admrNm;
	        	}
		        ssInfo = _this.findItemStoreStock(_this.postNo, _this.emdNm, _this.admrNm);
	        } catch(error) {
	        	console.log(error);	        	
			...
			if (ssInfo.result == "Y") {
				...
				$("#quickYn").val("Y");
				//$("input[name=o2oDlvSp][value='6']").click();	
				$("input[name=o2oDlvSp][value='6']").prop("checked", true);
        		...
        		// 2020.09.20 배송지변경시 Alert 추가 
        		if(_this.firstDeliveryChecked) {
	    			if (ssInfo.result == "N") {
	    				setTimeout(function(){alert("선택된 배송지의 오늘드림 매장 재고가 부족해요.");},300);
	    			} else {    				
	    				setTimeout(function(){alert("선택된 배송지는 일반배송 지역이에요.");},300);
	    			}
        		} else {
        			// 최초 배송지 체크시 불가지역인 경우.
        			$("#is24HCheckable").val("N");
		...
		if(_this.firstDeliveryChecked == false) {
			_this.firstDeliveryChecked = true;
		}
	},
	
	/**
	 * 1:일반주문, 2:24H주문, 3:오늘드림 영역 노출
	 * o2o_24h_chk_area 는 개별 조건에서 처리
	 * 2020-08-31 by kih
	 */
    orderDomAccess: function(chk) {
    	if (chk == "1") {		// 일반주문
    		$(".quick_area").hide();			// 배송지명, 공동현관출입방법, 
    		$(".o2o_dlv_area").hide();			// 배송구분
    		//$(".li_dueDt").hide();
    		$(".mutongjang").show();
    		$("#payMethodList").find("li:eq(4)").css("margin", "6px 0 0 0");
    		$("#payMethodList").find("li:eq(5)").css("margin", "6px 0 0 2%");
    		$("#quickYn").val("N");
    	...
    	} else if (chk == "2") { //24H주문배송				
    		$(".quick_area").show();
    		$(".o2o_dlv_area").hide();
    		//$(".li_dueDt").hide();
    		$(".mutongjang").hide();
    		$("#payMethodList").find("li:eq(4)").css("margin", "0 0 0 2%");
    		$("#payMethodList").find("li:eq(5)").css("margin", "6px 0 0 0");
    		$("#o2oDlvSp6").show();
            ...
            if($("#mhCnt").val() == 0){
                ...
                $("#mbrMemoCont_exist").find("li a[cd]").hide();
                $("#mbrMemoCont_exist").find("li a[cd]").eq(0).show();
                $("#mbrMemoCont_exist").find("li a[cd=O2O]").show();	// O2O
            }else{
                //최근 3개 메시지 노출
            	$("[name='mbrMemoCont']").val("");
                $("#msgCnt_exist").text($("[name='mbrMemoCont']").val().length);
                ...
                $("#mbrMemoCont_exist").find("li a[cd]").hide();
                $("#mbrMemoCont_exist").find("li a[cd]").eq(0).show();
                $("#mbrMemoCont_exist").find("li a[cd=MH]").show();		// MH
            }    		
    	} else if (chk == "3") {//오늘드림 주문
    		$(".quick_area").show();    		
    		$(".o2o_dlv_area").show();
    		$(".mutongjang").hide(); 	
    	} else {
    	...
    	console.log("orderDomAccess : " + chk);
    }	
};