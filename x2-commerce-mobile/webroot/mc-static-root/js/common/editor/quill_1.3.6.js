...
var Quill = function () {
  ...
  function Quill(container) {
    ...
    //add @20160919 jwlim callback 초기화
    this.callback = options.callback;
    this.imageResizeWidth = options.imageResizeWidth;
    this.tmpImageObj = options.tmpImageObj;
...
function expandConfig(container, userConfig) {
  ...
  //modify @20160919 jwlim 무조건 snow module을 이용하도록 userConfig.theme를 snow로 고정한다.
  userConfig.theme = "snow";
...
//add @20160919 jwlim callback 이 없을 경우 초기화 함.
  if (userConfig.callback == null) {
      userConfig.callback = {};
  }
  
  if (userConfig.imageResizeWidth == null) {
      userConfig.imageResizeWidth = 400;
  }

  if (userConfig.tmpImageObj == null) {
      userConfig.tmpImageObj = new Image();
  }
      ...
      handlers: {
        ...
        image: function image() {
          ...
          if (fileInput == null) {
            ...
            // canvas 생성 - 파일 선택 시 캔버스로 resize 및 transcoding 처리, canvas 
            var tmpCanvas = document.createElement('canvas');
            tmpCanvas.setAttribute('id', 'tmpCanvas');
            tmpCanvas.setAttribute('style', 'display:none');
            // input tag (file) 생성
...
//            fileInput.setAttribute('accept', 'image/png, image/gif, image/jpeg, image/bmp, image/x-icon');
            ...
            if( common.app.appInfo.isapp ){
                fileInput.setAttribute('multiple', 'multiple'); // dddd    
            }
            fileInput.classList.add('ql-image');
            
            var canvasImg = _this3.quill.tmpImageObj;
            //add @20160919 jwlim 이미지 업로드 추가
            var uploadImg = "";
            
            canvasImg.onload = function(e) {
                
                console.log('>>>>>>>>>>> canvansImg in' );
                
                $('#appxFileNm').val('');
                
                // 파일확장자 이름 알아내기
                if( fileInput.files.length > 0 ){
                    $('#appxFileNm').val(fileInput.files[0].name);
                    $('#imgSize').val(fileInput.files[0].size);
                }
                ...
                //500픽셀에 맞도록 배율 조정
                if (_this3.quill.imageResizeWidth == null) {
                    _this3.quill.imageResizeWidth = 505;
                }
                if (this.width > _this3.quill.imageResizeWidth) {
                    magnification = _this3.quill.imageResizeWidth / this.width;
                }
                 ...
                 * add @20160919 jwlim 이미지 업로드 추가
                 * TODO 
                 * 캔버스 이미지를 upload 처리 한다. 
                 * 이미지 업로드 처리 후 해당 이미지의 경로를 리턴받아 다음 quill lib에 insert 해준다.
                 */
                //uploadImg += canvas.toDataURL() + "%%%"; 
                uploadImg += canvas.toDataURL();
                //console.log('>>>>>> uploadImg : ' + uploadImg);
                if (_this3.quill.callback.uploadUrl != undefined && _this3.quill.callback.uploadUrl != null  ) {
                    if (typeof(_this3.quill.callback.uploadUrl) == "function") {
                        uploadImg = _this3.quill.callback.uploadUrl(uploadImg);
                    }
                }

                //TODO 업로드 이미지 URL을 넘겨받아 insert({ image : 넘겨받은 이미지 URL }) 처리한다.
                var range = _this3.quill.getSelection(true);
                _this3.quill.updateContents(
                        new _quillDelta2.default()
                        .retain(range.index)
                        .delete(range.length)
                        .insert({ image: uploadImg }, { "width" : "100%" , "class" : "a"}), 
                        _emitter2.default.sources.USER);
                //document.getElementById("tmpCanvas").remove();
            }
            ...
            fileInput.addEventListener('change', function () {
              if (fileInput.files != null && fileInput.files[0] != null) {
                  ...
                  console.log('file size : ' + fileInput.files.length );
                  ...
                  reader.onload = function(e) {
                      try {
                          ...
                          console.log('>>>> imgSize : ' + imgSize + ' | imgCur = ' + imgCur + ' | readyState = ' + e.target.readyState );
          ...
          /*alert('osType: ' + common.app.appInfo.ostype 
                 + '\n appver : ' + common.app.appInfo.appver );*/
