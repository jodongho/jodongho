(function(window, undefined) {
	...
	var UserAgentUtil = function(uastring, extensions) {
		...
		/**
		 * getting webview application name and version (oymall app)
		 */
		this.getAppInfo = function() {
		    ...
		    var tmpSplitArr = this.getUa().split(" ");
		    var itemArr;
		    if (tmpSplitArr.length > 0) {
		        for (var i = 0; i < tmpSplitArr.length; i++) {
		            if (tmpSplitArr[i].trim() == "") {
		                continue;
		            }
		            
		            itemArr = tmpSplitArr[i].trim().split(/\//g);
		            if (itemArr.length > 1) {
		                parseResult[itemArr[0]] = itemArr[1];
		            }
		        }
		        
		        if (parseResult.appver != null && parseResult.deviceid != null) {
		            parseResult.isapp = true;
		        }
		    }
