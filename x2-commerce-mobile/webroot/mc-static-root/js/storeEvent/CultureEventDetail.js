...
BeautyEvent.detail = {
	...
	init : function(){
	    ...
	    if((location.href).indexOf("getCultureEventDetail.do") > 0){
			/* 이벤트 호출시 활성화 */
	    	BeautyEvent.detail.initSns();
		}
	},

	checkBeautyJson : function(){
		...
		*/
		...
		common.Ajax.sendJSONRequest(
			  ...
			  ,  param
			  , BeautyEvent.detail._callback_checkBeautyJson
		);
	},
	_callback_checkBeautyJson : function(json){
		if(json.ret == "0"){
	...
	checkCultureJson : function(){
		...
		*/
		...
		common.Ajax.sendJSONRequest(
			  ...
			  , _baseUrl + "storeEvent/regBeautyNCultureEventJson.do"
			  ,  param
			  , BeautyEvent.detail._callback_checkCultureJson
		);
	},
	_callback_checkCultureJson : function(json){
		if(json.ret == "0"){
			alert("이벤트에 정상적으로 참여하셨습니다.");
		}else{
			alert(json.message);
