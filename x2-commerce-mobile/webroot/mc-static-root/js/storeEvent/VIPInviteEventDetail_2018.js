...
VIPEvent.detail = {
	...
	checkVIPInviteJson : function(){
		...
		common.Ajax.sendJSONRequest(
				"GET"
			  , _baseUrl + "storeEvent/regVIPInviteEventJson_2018.do"
			  ,  param
			  , VIPEvent.detail._callback_checkVIPInviteJson_2018
		);
	},
	_callback_checkVIPInviteJson_2018 : function(json){
		...
		if(json.ret == "0"){
			//VVIP :  mbrGradeCD == '5'
			alert("정상적으로 응모되었습니다. 7/13(금) 당첨자 안내를 기다려주세요!");
			location.href = url;
		}else{
			alert(json.message);
		}
    ...
    checkStore : function() {
        ...
        var Time = $.trim($("input[id='Time']:hidden").val());
